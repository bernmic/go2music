...
func ReadID3v1(f *os.File) (*Tag, error) {
	...
	if err == nil {
		...
		if err == nil && bytesRead == 128 && string(id3v1[0:3]) == "TAG" {
			tag := Tag{}
			tag.Title = strings.TrimRight(string(id3v1[3:33]), "\x00")
			tag.Artist = strings.TrimRight(string(id3v1[33:63]), "\x00")
			tag.Album = strings.TrimRight(string(id3v1[63:93]), "\x00")
			tag.Year = string(id3v1[93:97])
