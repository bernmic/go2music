...
func (db *DB) initializeArtist() {
	...
	if err != nil {
		...
		_, err := db.Exec("CREATE TABLE IF NOT EXISTS artist (id varchar(32), name varchar(255) NOT NULL, mbid varchar(36), PRIMARY KEY (id));")
		if err != nil {
		...
		_, err = db.Exec("CREATE INDEX artist_mbid ON artist (mbid)")
		if err != nil {
...
func (db *DB) CreateArtist(artist model.Artist) (*model.Artist, error) {
	...
	_, err := db.Exec(sanitizePlaceholder("INSERT INTO artist (id, name, mbid) VALUES(?, ?, ?)"), artist.Id, artist.Name, artist.Mbid)
	if err != nil {
		log.Error(err)
	}
	return &artist, err
...
func (db *DB) UpdateArtist(artist model.Artist) (*model.Artist, error) {
	_, err := db.Exec(sanitizePlaceholder("UPDATE artist SET name=?, mbid=? WHERE id=?"), artist.Name, artist.Mbid, artist.Id)
	return &artist, err
}
...
func (db *DB) FindArtistById(id string) (*model.Artist, error) {
	return fetchArtistRow(db.QueryRow(sanitizePlaceholder("SELECT id,name,mbid FROM artist WHERE id=?"), id))
}

// FindArtistByName get the artist with the given name
func (db *DB) FindArtistByName(name string) (*model.Artist, error) {
	return fetchArtistRow(db.QueryRow(sanitizePlaceholder("SELECT id,name,mbid FROM artist WHERE name=?"), name))
}
...
func (db *DB) FindAllArtists(filter string, paging model.Paging) ([]*model.Artist, int, error) {
	...
	rows, err := db.Query(sanitizePlaceholder("SELECT id, name, mbid FROM artist" + orderAndLimit))
	if err != nil {
...
func (db *DB) FindArtistsWithoutName() ([]*model.Artist, error) {
	rows, err := db.Query(sanitizePlaceholder("SELECT artist.id, artist.name, artist.mbid FROM artist WHERE artist.name IS NULL OR artist.name=''"))
	if err != nil {
