...
func Test_PagingSong(t *testing.T) {
	...
	paging.Size = 2
	s, l = createOrderAndLimitForSong(paging)
	if s != " ORDER BY song.title DESC LIMIT 2 OFFSET 0" {
		t.Error("Expected 'ORDER BY song.title DESC LIMIT 2 OFFSET 0'. got " + s)
	}
	if !l {
	...
	paging.Page = 1
	s, _ = createOrderAndLimitForSong(paging)
	if s != " ORDER BY song.title DESC LIMIT 2 OFFSET 2" {
		t.Error("Expected 'ORDER BY song.title DESC LIMIT 2 OFFSET 2'. got " + s)
	}
}
