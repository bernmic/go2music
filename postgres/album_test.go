...
func Test_PagingAlbum(t *testing.T) {
	...
	paging.Size = 2
	s, l = createOrderAndLimitForAlbum(paging)
	if s != " ORDER BY title DESC LIMIT 2 OFFSET 0" {
		t.Error("Expected 'ORDER BY title DESC LIMIT 2 OFFSET 0'. got " + s)
	}
	if !l {
	...
	paging.Page = 1
	s, _ = createOrderAndLimitForAlbum(paging)
	if s != " ORDER BY title DESC LIMIT 2 OFFSET 2" {
		t.Error("Expected 'ORDER BY title DESC LIMIT 2 OFFSET 2'. got " + s)
	}
}
