...
func Test_PagingUser(t *testing.T) {
	...
	paging.Size = 2
	s, _ = createOrderAndLimitForUser(paging)
	if s != " ORDER BY username DESC LIMIT 2 OFFSET 0" {
		t.Error("Expected 'ORDER BY username DESC LIMIT 2 OFFSET 0'. got " + s)
	}
	paging.Page = 1
	s, _ = createOrderAndLimitForUser(paging)
	if s != " ORDER BY username DESC LIMIT 2 OFFSET 2" {
		t.Error("Expected 'ORDER BY username DESC LIMIT 2 OFFSET 2'. got " + s)
	}
}
